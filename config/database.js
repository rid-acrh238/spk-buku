...
// KITA PAKSA LOGICNYA BIAR GAK SALAH BACA ENV
const db = mysql.createPool({
    ...
    // ⚠️ PAKSA SSL NYALA (TiDB Wajib SSL)
    ssl: {
        minVersion: 'TLSv1.2',
        rejectUnauthorized: true
    },

    // ⚠️ SETTING TIMEOUT EKSTRIM
    connectTimeout: 60000, // Tunggu 60 Detik
    waitForConnections: true,
    connectionLimit: 5,    // Jangan banyak-banyak buat serverless
    queueLimit: 0
});

// Cek koneksi di log saat server start (Biar ketahuan di Vercel Logs)
(async () => {
    try {
        const connection = await db.getConnection();
        console.log("✅ BERHASIL TEMBUS KE DATABASE TIDB!");
        connection.release();
    } catch (err) {
        console.error("❌ GAGAL KONEK TIDB. DETAIL:", err.message);
    }
})();
